...
class AmericanPricing:
    ...
    @memoize
    def get_all_paths(
        self, spot_pct_noise: float, num_paths: int, num_dt: int
    ) -> np.ndarray:
        """
        ...
        for i in range(num_paths):
            ...
            for t in range(num_dt):
                m, v = get_future_price_mean_var(
                    paths[i, t], t, dt, self.lognormal, self.ir, self.isig
                )
    ...
    def get_ls_price(
        ...
        payoff_expiry = np.array(
            [self.payoff(self.expiry, paths[i, :]) for i in range(num_paths)]
        )
        cashflow = np.maximum(payoff_expiry, 0)
        dt = self.expiry / num_dt

        stprcs = np.arange(100.0)
        final = [
            (p, self.payoff(self.expiry, np.append(np.zeros(num_dt), p)))
            for p in stprcs
        ]
        ...
        for step in range(num_dt - 1, 0, -1):
            ...
            cashflow *= disc
            payoff = np.array(
                [self.payoff(t, paths[i, : (step + 1)]) for i in range(num_paths)]
            )
            indices = np.where(payoff > 0)[0]
            if len(indices) > 0:
                x_vals = np.array(
                    [
                        [f(step, paths[i, : (step + 1)]) for f in feature_funcs]
                        for i in indices
                    ]
                )
                y_vals = cashflow[indices]
                ...
                cp = [
                    weights.dot([f(step, prsq) for f in feature_funcs])
                    for prsq in prsqs
                ]
    ...
    def state_reward_gen(
        self, state: StateType, action: ActionType, num_dt: int
    ) -> Tuple[StateType, float]:
        ind, price_arr = state
        delta_t = self.expiry / num_dt
        t = ind * delta_t
        reward = (
            (np.exp(-self.ir(t)) * self.payoff(t, price_arr))
            if (action and ind <= num_dt)
            else 0.0
        )
        m, v = get_future_price_mean_var(
            price_arr[-1], t, delta_t, self.lognormal, self.ir, self.isig
        )
        norm_draw = np.random.normal(m, np.sqrt(v))
        ...
        for path_num, path in enumerate(paths):
            ...
            while step <= num_dt:
                ...
                if step == num_dt:
                    ...
                    continue_price = params.dot(
                        [f(step, price_seq) for f in feature_funcs]
                    )
                step += 1
    ...
    def get_lspi_price(
        ...
        for _ in range(num_iters):
            ...
            for path_num, path in enumerate(paths):
                ...
                for step in range(num_dt):
                    ...
                    a_mat += np.outer(phi_s, phi_s - phi_sp * disc)
        ...
        return self.get_price_from_paths_and_params(
            self.get_all_paths(0.0, num_paths, num_dt), params, num_dt, feature_funcs
        )
